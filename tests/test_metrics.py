...
class TestMetricsCollector:
    ...
    def test_timer_histogram_includes_min_and_max(self):
        self.metrics.start_timer("operation")
        import time
        time.sleep(0.01)
        duration = self.metrics.stop_timer("operation")

        histogram = self.metrics.snapshot()["histograms"]["operation"]
        assert histogram["count"] == 1
        assert histogram["min"] == duration
        assert histogram["max"] == duration
