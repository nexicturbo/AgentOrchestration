...
class TestMetricsCollector:
    ...
    @pytest.mark.parametrize("value", ["85.5", None, object(), True])
    def test_gauge_rejects_non_numeric_values(self, value):
        self.metrics.gauge("memory.usage", 85.5)

        with pytest.raises(TypeError, match="Gauge values must be numeric"):
            self.metrics.gauge("memory.usage", value)

        snapshot = self.metrics.snapshot()
        assert snapshot["gauges"]["memory.usage"] == 85.5

    def test_gauge_accepts_integer_values_as_numeric(self):
        self.metrics.gauge("queue.depth", 3)

        snapshot = self.metrics.snapshot()
        assert snapshot["gauges"]["queue.depth"] == 3.0
