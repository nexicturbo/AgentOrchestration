...
class TestConfig:
    ...
    def test_env_override_non_boolean_stays_string(self, monkeypatch):
        monkeypatch.setenv("AO_FEATURE_ENABLED", "0")
        monkeypatch.setenv("AO_FEATURE_MODE", "falsehood")
        config = Config()
        assert config.get("feature.enabled") == "0"
        assert config.get("feature.mode") == "falsehood"
