...
class AuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(
        ...
        if is_protected_path(request.url.path):
            decision = self.validator.authenticate(request)
            if not decision.allowed:
                return Response(status_code=401, content=decision.reason)
        return await call_next(request)


def is_protected_path(path: str) -> bool:
    canonical_path = path.rstrip("/") or "/"
    if canonical_path == "/api/v2/auth/token":
        return False
    return canonical_path == "/api/v2" or canonical_path.startswith("/api/v2/")
