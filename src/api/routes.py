...
def _validate_non_blank(value: str, field: str) -> str:
    if not isinstance(value, str):
        raise _validation_error(f"{field} must be a string")
    normalized = value.strip()
    if not normalized:
        raise _validation_error(f"{field} must not be blank")
    return normalized


def _validate_agent_id(agent_id: str) -> str:
    normalized = _validate_non_blank(agent_id, "agent_id")
    try:
        return str(UUID(normalized))
    except ValueError:
        raise _validation_error("agent_id must be a UUID")


def _validate_status(status: Optional[str]) -> Optional[AgentStatus]:
    if status is None:
        return None
    try:
        return AgentStatus(status)
    except ValueError:
        allowed = ", ".join(item.value for item in AgentStatus)
        raise _validation_error(f"status must be one of: {allowed}")
...
async def register_agent(
    ...
    name = _validate_non_blank(name, "name")
    agent_type = _validate_non_blank(agent_type, "agent_type")
