...
from .artifacts import ArtifactIngestionError, artifact_ingestion_service
...
async def upload_run_artifact(
    ...
    try:
        artifact_ingestion_service.validate_request(
            run_id,
            artifact_name,
            request.headers.get("content-length"),
        )
        body = await request.body()
        record = artifact_ingestion_service.ingest(
            run_id,
            artifact_name,
            body,
        )
