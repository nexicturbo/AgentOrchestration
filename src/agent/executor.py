...
class AgentExecutor:
    ...
    async def execute(
        ...
        async with self._semaphore:
            ...
            try:
                ...
                self._store_terminal_result(execution_id, result)
            except Exception as e:
                self._store_terminal_result(execution_id, {"error": str(e)})
            finally:
    ...
    def _store_terminal_result(self, execution_id: str, result: Any) -> None:
        self._results.setdefault(
            execution_id,
            self._json_serializable_result(execution_id, result),
        )

    def _json_serializable_result(self, execution_id: str, result: Any) -> Any:
        try:
