...
class ResourceLimits:
    def __init__(
        ...
        values = Config.validate_resource_limits({
            "cpu_time": cpu_time,
            "memory_mb": memory_mb,
            "disk_mb": disk_mb,
        })
        self.cpu_time = values["cpu_time"]
        self.memory_mb = values["memory_mb"]
        self.disk_mb = values["disk_mb"]
...
class AgentSandbox:
    ...
    def apply_limits(self, agent_id: str, limits: ResourceLimits) -> None:
        ...
        try:
            resource.setrlimit(
                resource.RLIMIT_CPU,
                (values["cpu_time"], values["cpu_time"]),
            )
            mem_bytes = values["memory_mb"] * 1024 * 1024
