...
def bounded_log_tail(value: str) -> int:
    try:
        tail = int(value)
    except ValueError as exc:
        raise argparse.ArgumentTypeError("tail must be an integer") from exc

    if tail < 1 or tail > MAX_LOG_TAIL_LINES:
        raise argparse.ArgumentTypeError(
            f"tail must be between 1 and {MAX_LOG_TAIL_LINES} lines"
        )
    return tail
...
def cli():
    ...
    logs_parser.add_argument(
        "--tail",
        "-t",
        type=bounded_log_tail,
        default=50,
        help=f"Number of lines to fetch (1-{MAX_LOG_TAIL_LINES})",
    )
    ...
    if args.command == "init":
        ...
        print(
            f"Fetching last {args.tail} log lines "
            f"for agent: {args.agent_id}"
        )
    else:
        parser.print_help()
        sys.exit(1)
