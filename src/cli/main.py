...
def cli():
    ...
    if args.command == "init":
        ...
        decision = MigrationRolloutGate().evaluate(manifest)
        if not decision.allowed:
            print(f"Rollout blocked: {decision.reason}", file=sys.stderr)
            sys.exit(2)

        print("Migrations complete; releasing application traffic")
